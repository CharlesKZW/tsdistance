...
def kumarhassebrook(x, y):
    ...
    if len(x) != len(y):
        return -1
    return np.sum(np.multiply(x, y)) / np.subtract(
        np.add(np.sum(np.square(x)), np.sum(np.square(y))), np.sum(np.multiply(x, y))
    )
...
def jaccard(x, y):
    ...
    if len(x) != len(y):
        return -1
    return np.sum(np.square(np.subtract(x, y))) / np.sum(
        np.square(x) + np.square(y) + np.multiply(np.add(x, y), -1)
    )
...
def topsoe(x, y):
    ...
    for i in range(len(x)):
        sum += (x[i] * (math.log(2 * x[i]) - logxy[i])) + (
            y[i] * (math.log(2 * y[i]) - logxy[i])
        )
    return sum
...
def jensen_shannon(x, y):
    ...
    for i in range(len(x)):
        ...
        if y[i] <= 0:
            return -1
        sum += x[i] * (math.log(2 * x[i]) - logxy[i]) + y[i] * (
            math.log(2 * y[i]) - logxy[i]
        )
...
def jensen_difference(x, y):
    ...
    for i in range(len(x)):
        ...
        if x[i] <= 0:
            return -1
        sum += (x[i] * math.log(x[i]) + y[i] * math.log(y[i])) / 2 - xyavg[
            i
        ] * math.log(xyavg[i])
